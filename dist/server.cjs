...
function preprocessText(text) {
  if (!text) return [];
  const tokens = text.toLowerCase().replace(/[^\w\s+#_-]/g, " ").split(/\s+/);
  const processed = [];
...
var indexedJobs = [];
var idfMap = /* @__PURE__ */ new Map();
async function generateAIContent(model, systemInstruction, messages, responseMimeType) {
...
async function startServer() {
  ...
  try {
    const csvPath = import_path.default.join(process.cwd(), "data", "preprocessed_jobs.csv");
    ...
    if (import_fs.default.existsSync(csvPath)) {
      ...
      for (let i = 1; i < rows.length; i++) {
        ...
        const item = {
          ...
          rating: parseFloat(job.rating) || 0,
          ...
          All: job.All || "",
          tokens: [],
          tfidfVec: /* @__PURE__ */ new Map(),
          tfidfNorm: 0,
          tfVec: /* @__PURE__ */ new Map(),
          tfNorm: 0
        };
      ...
      jobs.forEach((job) => {
        ...
        const tfidfVec = /* @__PURE__ */ new Map();
        let tfidfNormSq = 0;
        tf.forEach((count, t) => {
  ...
  function recommendJobs(resumeText, targetLocations = [], limit = 10) {
    ...
    const resumeTf = /* @__PURE__ */ new Map();
    resumeTokens.forEach((t) => resumeTf.set(t, (resumeTf.get(t) || 0) + 1));
    const resumeTfidfVec = /* @__PURE__ */ new Map();
    ...
    for (const job of indexedJobs) {
      ...
      const tfidfSim = resumeTfidfNorm > 0 && job.tfidfNorm > 0 ? tfidfDot / (resumeTfidfNorm * job.tfidfNorm) : 0;
      let cvDot = 0;
      ...
      const cvSim = resumeCountNorm > 0 && job.tfNorm > 0 ? cvDot / (resumeCountNorm * job.tfNorm) : 0;
  ...
  app.post("/api/interview/chat", async (req, res) => {
    try {
      ...
      if (mode) {
        ...
        if (mode === "companion") {
          dynamicSystemInstruction = "You are a warm, casual, and highly empathetic companion chatbot. Your goal is to talk with the user about anything on their mind, help them relax, have casual conversations, share jokes, or provide a safe space to vent. Keep your tone lighthearted, friendly, caring, and conversational. Do not behave like an interviewer; be a supportive friend. Keep your responses relatively short, conversational, and friendly.";
        } else {
          dynamicSystemInstruction = `You are a world-class AI Interviewer and Technical Coach. You are conducting a mock technical interview for a ${experienceLevel || "Beginner"} level role at ${company || "a tech company"} on the topic of ${topic || "general technical topics"}. ${resumeContext ? `Here is the candidate's resume/background for context: ${resumeContext}

` : ""}Your job is to conduct an interactive mock interview. When asking questions, ask exactly one question at a time. If the user asks for a question (e.g., starting with QUESTION_REQUEST), respond directly by asking a single high-quality question suited for their target experience level and topic. Do not provide answers, solutions, or lists of multiple questions. When the user provides an answer (starting with USER_ANSWER), acknowledge their input with brief constructive feedback or follow-up question, but DO NOT list the answer. If the interview is complete (starting with INTERVIEW_COMPLETE), provide a comprehensive, deep analysis of their interview performance. Include: Overall Score (0-100), Strengths (as a clear bulleted list), Areas for Improvement (as a clear bulleted list), and separate rating numbers (from 1 to 10) for Communication, Technical, and Confidence. Format the final analysis clearly with standard labels (e.g., 'Overall Score: XX', 'Strengths:', 'Areas for Improvement:', 'Communication: X/10', 'Technical: X/10', 'Confidence: X/10', followed by a 'Detailed Feedback' section) so that a regex script can easily parse the scores and bullet points.`;
        }
        const geminiHistory = (messages || []).filter((m) => m.sender === "user" || m.sender === "bot").map((m) => ({
          ...
          if (response.ok) {
            ...
            if (data.status === "ok" && Array.isArray(data.articles) && data.articles.length > 0) {
              const formattedArticles = data.articles.map((item, index) => {
                ...
                const publishedDateStr = item.publishedAt ? new Date(item.publishedAt).toLocaleDateString(void 0, { month: "short", day: "numeric" }) : "Recent";
                return {
  ...
  app.post("/api/research/youtube", async (req, res) => {
    try {
      ...
      try {
        ...
        const data = await response.json();
        const videos = (data.items || []).filter((item) => item.id?.videoId).map((item) => ({
          id: item.id.videoId,
...
${resumeText}` }] }],
      ...
      if (!parsedData) {
        ...
        let detectedField = "Web Development";
        let maxOverlap = 0;
        const lowerResume = resumeText.toLowerCase();
        Object.keys(domainKeywords).forEach((field) => {
          let overlap = 0;
        ...
        parsedData = {
          ...
          cand_level: resumeText.length < 2e3 ? "Fresher" : resumeText.length < 5e3 ? "Intermediate" : "Experienced",
      ...
      const track = parsedData.reco_field || "Web Development";
      const fullKeywords = domainKeywords[track] || domainKeywords["Web Development"];
      const existingLower = parsedData.skills.map((s) => s.toLowerCase());
      const recSkills = fullKeywords.filter((k) => !existingLower.includes(k.toLowerCase())).slice(0, 8).map((k) => k.charAt(0).toUpperCase() + k.slice(1));
      ...
      const tips = allSections.map((sect) => {
        ...
        if (!isMissing) {
          ...
          if (sect === "Achievements") message = "Boost your chance by adding key Achievements\u{1F3C5} validating your background.";
          if (sect === "Projects") message = "Critical: Add technical Projects\u{1F468}\u200D\u{1F4BB}. It proves real hands-on implementation capabilities.";
          return { type: symbol, text: message };
        }
      });
      return res.json({
